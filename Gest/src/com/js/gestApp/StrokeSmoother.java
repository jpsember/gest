...
public class StrokeSmoother {
  ...
  public StrokeSet getSmoothedSet() {
    if (mSmoothed == null) {
      ...
      mSmoothed.inheritMetaDataFrom(mSet);
      mSmoothed.freeze();
    }
    return mSmoothed;
  }
  ...
  private Point calcStrokeVelocity(Stroke stroke, int index) {
    ...
    DataPoint e0 = stroke.get(index - 1);
    DataPoint e1 = stroke.get(index);
    DataPoint e2 = stroke.get(index + 1);
  ...
  private Stroke smoothStroke(Stroke origStroke) {
    ...
    for (int strokeIndex = 0; strokeIndex < origStroke.size(); strokeIndex++) {
      DataPoint currentPoint = origStroke.get(strokeIndex);
